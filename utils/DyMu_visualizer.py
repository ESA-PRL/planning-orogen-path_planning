...
costMap = np.loadtxt(open("GlobalCostMap_0.txt", "rb"), skiprows=0)
costMap[np.where(costMap==-1)] = np.nan

total_cost = np.loadtxt(open("TotalCostMap_0.txt", "rb"), skiprows=0)
total_cost[np.where(total_cost==-1)] = np.nan

path = np.loadtxt(open("Path_0.txt", "rb"), skiprows=0)
