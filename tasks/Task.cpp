...
bool Task::startHook()
{
    ...
    planner->initGlobalLayer(_global_res, _local_res, map_size_X, map_size_Y, global_offset);
    ...
    if (!planner->computeCostMap(
            cost_data, slope_values, locomotion_modes, elevationMatrix, terrain_matrix))
    ...
    if (_write_results) localTimeFile.open("LocalTimeValues.txt");
    ...
    if (_pose.read(pose) == RTT::NewData)
    {
        ...
        if (state() == WAITING_FOR_POSE)
        {
            if (planner->computeTotalCostMap(wRover))
            {
                ...
                if (_write_results) writeGlobalResults();
                state(PATH_COMPUTED);
            }
            else
            {
            ...
            if (set_travmap)
            {
                ...
                for (uint j = 0; j < height; j++)
                    for (uint i = 0; i < width; i++)
                        if (sqrt(pow((float)i - obstacle_center_x, 2)
...
void Task::writeGlobalResults()
{
    ...
    std::string total_cost_filename =
        std::string("TotalCostMap_") + std::to_string(num_globalpp_executions) + ".txt";
    std::string global_cost_filename =
        std::string("GlobalCostMap_") + std::to_string(num_globalpp_executions) + ".txt";
    std::string hazard_density_filename =
        std::string("HazardDensityMap_") + std::to_string(num_globalpp_executions) + ".txt";
    std::string trafficability_filename =
        std::string("TrafficabilityMap_") + std::to_string(num_globalpp_executions) + ".txt";
    std::string path_filename =
        std::string("Path_") + std::to_string(num_globalpp_executions) + ".txt";
    ...
    for (uint k = 0; k < trajectory2D.size(); k++)
        path_file << trajectory2D[k].position[0] << " " << trajectory2D[k].position[1] << "\n";
...
void Task::writeLocalResults()
{
    ...
    std::string risk_filename =
        std::string("RiskMap_") + std::to_string(num_globalpp_executions) + ".txt";
    std::string deviation_filename =
        std::string("DeviationMap_") + std::to_string(num_globalpp_executions) + ".txt";
